...
def main():
    ...
    name = f'logs/cv_embc2_aggressive_weight_patient_resnet_multi_{num_folds}'
    ...
    for fold, (df_train, df_val, df_test) in enumerate(split):
        ...
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate),
                      loss=focal_loss_with_class_weights(alpha=weights, gamma=2.0),
                      metrics=[CategoricalAccuracy(name='cat_accuracy'), Precision(name='precision'),
                               CombinedRecall()])

        name_fold = name + f'fold_{fold}'
        checkpoint_dir, callbacks = get_recall_early(name_fold, learning_rate)
