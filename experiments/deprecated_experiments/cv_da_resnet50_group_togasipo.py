...
def main():
    ...
    learning_rate = 1e-4
    num_folds = 5
    name = f'resnet50_da_togasipo_cv_{num_folds}'
    ...
    for fold, (train_idx, val_idx, test_idx) in enumerate(split):
        ...
        tf_val_df = get_tf_eggim_patch_dataset(df_togas.loc[val_idx],
                                               num_classes=3,
                                               preprocess_fn=tf.keras.applications.resnet.preprocess_input)
        tf_test_df = get_tf_eggim_patch_dataset(df_togas.loc[test_idx],
                                                num_classes=3,
                                                preprocess_fn=tf.keras.applications.resnet.preprocess_input)
